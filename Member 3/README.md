...
Department of Computer Science and Engineering Netrokona University,Netrokona
...
users	id, role	name, email, password(hashed), phone, role(owner/tenant/admin)
listings	id, owner_id	title, description, location, price, room_type, bedrooms, amenities, status
listing_images	id, listing_id	image_url
requests	id, listing_id, tenant_id	status(pending/accepted/rejected), created_at
wishlist	id, tenant_id, listing_id	just a reference table to store saved listings
